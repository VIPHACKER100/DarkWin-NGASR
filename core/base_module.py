...
class BaseModule:
    ...
    def add_finding(self, vuln_type: str, severity: str, endpoint: str, 
        ...
        with SessionLocal() as db:
            ...
            db.add(finding)
            db.commit()
            self.log(f"💎 New Finding: [{severity}] {vuln_type}")
            
            # Trigger background verification
            asyncio.create_task(self._verify_finding(finding.id, vuln_type, endpoint, payload))

    async def _verify_finding(self, finding_id: int, vuln_type: str, endpoint: str, payload: Optional[str]):
        """Internal helper to run verification."""
        ...
        if is_verified:
            with SessionLocal() as db:
                ...
                if f:
                    f.verified = True
                    db.commit()
                    self.log(f"✅ Verified Finding ID {finding_id}: {vuln_type}")
                    
                    # Notify on verified critical/high findings
